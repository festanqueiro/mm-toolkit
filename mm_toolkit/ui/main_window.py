...
from mm_toolkit.ui.video_creator import VideoCreatorTab
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.video_creator = VideoCreatorTab(self.settings)
        ...
        self.tabs.addTab(self.video_creator, material_icon("music_video", icon_color), "Video Creator")
        ...
        self.video_creator.job_completed.connect(self.add_history)
        self.clips.job_completed.connect(self.add_history)
        self.converter.job_completed.connect(self.add_history)
        self.video_creator.view_in_history.connect(self.show_latest_in_history)
    ...
    def apply_app_settings(self) -> None:
        ...
        if default_output and Path(default_output).is_dir():
            if not self.video_creator.output.path:
                self.video_creator.output.set_path(default_output)
    ...
    def load_history_job(self, record: dict) -> None:
        if record.get("tool") == "promo":
            self.video_creator.music.set_path(record.get("source", ""))
            self.video_creator.cover.set_path(record.get("cover", ""))
            self.video_creator.output.set_path(record.get("output", ""))
            if record.get("effects"):
                self.video_creator.effects_panel.apply_state(record["effects"])
            else:
                self.video_creator.effects_panel.rows["bass_blur"].checkbox.setChecked(
                    record.get("bass_effect", True)
                )
            self.video_creator.video_fade.setChecked(record.get("video_fade", True))
            self.video_creator.audio_fade.setChecked(record.get("audio_fade", True))
            self.video_creator.mute_original_video_audio.setChecked(record.get("mute_original_video_audio", True))
            stored_tracks = {item.get("path"): item for item in record.get("tracks", [])}
            for row in range(self.video_creator.promo_tracks.rowCount()):
                path = self.video_creator.promo_tracks.item(row, 0).data(Qt.ItemDataRole.UserRole)
                if path in stored_tracks:
                    self.video_creator.promo_tracks.cellWidget(row, 1).setText(format_timestamp(stored_tracks[path].get("start", 0)))
                    self.video_creator.promo_tracks.cellWidget(row, 2).setValue(stored_tracks[path].get("duration", 60))
            self.video_creator.analysis_status.setText("✓ Loaded saved per-track timings.")
            self.video_creator.fps.setValue(record.get("fps", 24))
            profile = record.get("profile")
            self.video_creator.profile.setCurrentIndex(max(0, self.video_creator.profile.findData(tuple(profile) if profile else None)))
    ...
    def closeEvent(self, event):  # noqa: N802, ANN001
        promo_running = self.video_creator.worker and self.video_creator.worker.isRunning()
        analysis_running = self.video_creator.analysis_worker and self.video_creator.analysis_worker.isRunning()
        ...
        if promo_running or analysis_running or clips_running or converter_running:
            ...
            event.ignore()
            return
        self.video_creator.stop_promo_preview()
        event.accept()
